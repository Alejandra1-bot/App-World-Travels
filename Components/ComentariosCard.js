...
export default function ComentarioCard({ comentario, onEdit, onDelete, userRole }) {
  const inicial = comentario.usuario ? comentario.usuario.charAt(0).toUpperCase() : "?";
  ...
  return (
    <TouchableOpacity style={styles.card}>
      ...
      <View style={styles.info}>
        <Text style={styles.nombre}>{comentario.usuario}</Text>
        <View style={styles.row}>
          <Ionicons name="chatbubbles-outline" size={16} color="#555" />
          <Text style={styles.detalle}> {comentario.texto}</Text>
        </View>
        <View style={styles.row}>
          <Ionicons name="star-outline" size={16} color="#555" />
          <Text style={styles.detalle}> Calificación: {comentario.calificacion}</Text>
        </View>
      </View>
